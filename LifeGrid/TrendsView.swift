                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            // Create a namespace for the preview
                            ZStack {
                                // Use a GeometryReader to handle the namespace
                ...
                Chart {
                    ...
                    if weeklyAverages.count > 1 {
                        ...
                        if let startPoint = trendLine.first, let endPoint = trendLine.last {
                            LineMark(
                                x: .value("Week", startPoint.weekStart, unit: .weekOfYear),
                                y: .value("Trend", startPoint.average)
                            )
                            .foregroundStyle(Color.red.opacity(0.6))
                            
                            LineMark(
                                x: .value("Week", endPoint.weekStart, unit: .weekOfYear),
